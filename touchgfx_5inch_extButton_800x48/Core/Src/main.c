...
/* USER CODE BEGIN PV */
__IO uint8_t User_ButtonState0=0;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	...
	{
		User_ButtonState1=1; //Key2
	}
	if(GPIO_Pin==GPIO_PIN_3)
	{
		User_ButtonState0=1; //Key1
	}
...
void MPU_Config(void) {
	...
	HAL_MPU_Disable();		// Disable the MPU first

	MPU_InitStruct.Enable           = MPU_REGION_ENABLE;
	MPU_InitStruct.BaseAddress      = ((uint32_t)0xC0000000);
	MPU_InitStruct.Size             = MPU_REGION_SIZE_32MB;
	MPU_InitStruct.AccessPermission = MPU_REGION_FULL_ACCESS;//MPU_REGION_FULL_ACCESS;
